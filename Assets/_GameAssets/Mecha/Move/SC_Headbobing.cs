...
public class SC_Headbobing : MonoBehaviour
{
    ...
    public void HeadBob()
    {
        ...
        //float f_CurAmplitude = (Mathf.Cos((rb_Cockpit.velocity.magnitude * f_HeadBobingTimer)/f_FrequenceFactor) * f_MaxAmplitude) - f_MaxAmplitude + f_InitialHeight;

        //float f_CurAmplitude = (Mathf.Cos(f_HeadBobingTimer * f_FrequenceFactor ) * f_MaxAmplitude) - f_MaxAmplitude + f_InitialHeight;
        ...
        f_HeadBobingTimer += Time.deltaTime * Mathf.Floor(rb_Cockpit.velocity.magnitude / 10);
